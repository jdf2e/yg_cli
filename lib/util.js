...
const util = {
    ...
    async cmdArgs(argsArr) {
        ...
        ygConfigArgs.map((v, i) => {
            ...
            if (v == "-s") {
                util.ygConfig.s = ygConfigArgs[i + 1];
            }
        });

        // 打开浏览器 open
        if (util.ygConfig.open) {
            open(`${config.DOMAIN}/yg_src/${util.ygConfig.puuid}`)
            open(`http://yg.jd.com:${util.ygConfig.outerPort}`);
        }
        ...
        // 固化版本
        if (util.ygConfig.s) {
            console.log(`开始保存新模板:${util.ygConfig.s}`);
            util.saveTemplate(util.ygConfig.s)
            .then(body => {
                if (body.code === 0) {
                    console.log('模板保存成功');
                } else if (body.code === 1) {
                    console.log('模板已存在, 请更名保存');
                }
            });
            return;
        }
        ...
        const socket = util.socket = io(config.DOMAIN);

        socket.on('connect', function (err) {
            //console.log("socket connected.");
            if (err) throw err;
            interact.start(socket);
            util.saveYgConfig();
            if (userConfigArgs.length > 0) {
                ...
                util.tarFolder(util.currentFolder, folders, f).then(d => {
                    ...
                    //console.log("At URL:", `${config.DOMAIN}/yg_src/${util.ygConfig.id}`);
                })
            } else {
            ...
            let watcher = util.watcher = util.startWatch();
        ...
        socket.on('disconnect', function () {
            console.log("----------------------------------------------------------");
            console.log("目录浏览：", `${config.DOMAIN}/yg_src/${util.ygConfig.puuid}/`);
            console.log("云构 URL：", `http://yg.jd.com:${util.ygConfig.outerPort}`);

            if (util.ygConfig.open && userConfigArgs.length <= 0) {
                setTimeout(d => {
                    process.exit();
                }, 2000);
            } else {
    ...
    addignore() {
        ...
        if (fs.existsSync(ignoreFile)) {
            if (!fs.readFileSync(".gitignore", "utf-8").includes(".ygconfig")) {
    ...
    startWatch() {
        let changeArr = [];
        let timeOut;
        let watcher = watch(currentFolder, {
            recursive: true,
            filter: f => !/\.tgz$/.test(f),
        }, function (evt, name) {
            changeArr.push(name);
    ...
    download(folder = "") {
        return new Promise((resolve, reject) => {
            ...
            request(`${config.DOMAIN}/download?puuid=${util.ygConfig.puuid}&folder=${folder}`).pipe(
    ...
    saveTemplate(name = "") {
        ...
        return new Promise((resolve, reject) => {
            request(`${config.DOMAIN}/savetpl?puuid=${util.ygConfig.puuid}&tplname=${name}`, (err, res, body) => {
